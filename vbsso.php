 ...
 */
if (get_site_option(VBSSO_NAMED_EVENT_FIELD_EDIT_PROFILE_IN_VBULLETIN, 1) && get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_PROFILE_URL, '') != '') {
...
function vbsso_load_profile_hook() {
    ...
    if (!in_array('administrator', $current_user->roles)) {
        wp_redirect(get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_PROFILE_URL));
    }
}

if (get_site_option(VBSSO_NAMED_EVENT_FIELD_SHOW_VBULLETIN_AUTHOR_PROFILE, 1) && get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_PROFILE_URL, '') != '') {
...
function vbsso_author_link_hook($link, $author_id) {
    ...
    return get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_PROFILE_URL, '') . md5(strtolower($email));
}

/**
 * Register Url Filter
 */
if (vbsso_get_platform_config_property(SHAREDAPI_PLATFORM_WORDPRESS, VBSSO_CONFIG_PROPERTY_OVERRIDE_LINKS, true) && get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_REGISTER_URL, '') != '') {
    add_filter('register', 'vbsso_register_url_hook');
}
function vbsso_register_url_hook() {
    if (!is_user_logged_in()) {
        if (get_option('users_can_register') || get_site_option('vbsso_ignore_membership', 1))
            $link = '<li><a href="' . sharedapi_url_add_destination(get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_REGISTER_URL, ''), 'server', '', get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LID, '')) . '" rel="nofollow">' . __('Create your account') . '</a></li>';
        else
            $link = '';
    } else {
 ...
 * Login Url Filter
 */
if (vbsso_get_platform_config_property(SHAREDAPI_PLATFORM_WORDPRESS, VBSSO_CONFIG_PROPERTY_OVERRIDE_LINKS, true) && get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LOGIN_URL, '') != '') {
    add_filter('login_url', 'vbsso_login_url_hook');
}
function vbsso_login_url_hook() {
    return sharedapi_url_add_destination(get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LOGIN_URL, ''), 'server', '', get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LID, ''));
}

/**
 * Logout Url Filter
 */
if (vbsso_get_platform_config_property(SHAREDAPI_PLATFORM_WORDPRESS, VBSSO_CONFIG_PROPERTY_OVERRIDE_LINKS, true) && get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LOGOUT_URL, '') != '') {
    add_filter('logout_url', 'vbsso_logout_url_hook');
}
function vbsso_logout_url_hook() {
    return sharedapi_url_add_destination(get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LOGOUT_URL, ''), 'server', '', get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LID, ''));
}

/**
 * Lost Password Url Filter
 */
if (vbsso_get_platform_config_property(SHAREDAPI_PLATFORM_WORDPRESS, VBSSO_CONFIG_PROPERTY_OVERRIDE_LINKS, true) && get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LOSTPASSWORD_URL, '') != '') {
    add_filter('lostpassword_url', 'vbsso_lostpassword_url_hook');
}
function vbsso_lostpassword_url_hook() {
    return sharedapi_url_add_destination(get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LOSTPASSWORD_URL, ''), 'server', '', get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LID, ''));
}
...
function vbsso_profile_personal_options_hook() {
    global $current_user;
    if (!in_array('administrator', $current_user->roles)) {
        echo '<link rel="stylesheet" type="text/css" href="' . VBSSO_WORDPRESS_PLUGIN_URL . 'assets/css/profile-overrides.css">';
    }
}
 ...
 * Get Avatar Filter
 */
if (get_site_option(VBSSO_NAMED_EVENT_FIELD_FETCH_AVATARS, 1) && get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_AVATAR_URL, '') != '') {
...
function vbsso_get_avatar_hook($avatar, $id_or_email, $size = '96', $default = '', $alt = false) {
    $avatar_url = get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_AVATAR_URL, '');
    ...
    $email_hash = !empty($email) ? md5(strtolower($email)) : '';
...
function vbsso_get_vb_post_data($link, $fields = false) {
    $baa_username = sharedapi_decode_data(
        get_site_option(VBSSO_NAMED_EVENT_FIELD_API_KEY, SHAREDAPI_DEFAULT_API_KEY), get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_BAA_USERNAME, null)
    );

    $baa_password = sharedapi_decode_data(
        get_site_option(VBSSO_NAMED_EVENT_FIELD_API_KEY, SHAREDAPI_DEFAULT_API_KEY), get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_BAA_PASSWORD, null)
    );
...
function vbsso_get_vb_usergroups() {
    return vbsso_get_vb_post_data(get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_USERGROUPS_URL));
}

function vbsso_get_vb_user_unread_stats() {
    global $current_user;
    $data = vbsso_get_vb_post_data(
        get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_USER_UNREAD_STATS_URL, '')
        . get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LID, '') . '&id=' . md5(strtolower($current_user->data->user_email))
    );

    if (is_object($data) && isset($data->data)) {
        $data = sharedapi_accept_data(get_site_option(VBSSO_NAMED_EVENT_FIELD_API_KEY, SHAREDAPI_DEFAULT_API_KEY), $data->data);
    }
...
function vbsso_options() {
    ...
    if (isset($_POST['submit_options']) && $_POST['submit_options'] == 1) {
        ...
        update_site_option(VBSSO_NAMED_EVENT_FIELD_FETCH_AVATARS, isset($_POST[VBSSO_NAMED_EVENT_FIELD_FETCH_AVATARS]));
        update_site_option(VBSSO_NAMED_EVENT_FIELD_EDIT_PROFILE_IN_VBULLETIN, isset($_POST[VBSSO_NAMED_EVENT_FIELD_EDIT_PROFILE_IN_VBULLETIN]));
        update_site_option(VBSSO_NAMED_EVENT_FIELD_SHOW_VBULLETIN_AUTHOR_PROFILE, isset($_POST[VBSSO_NAMED_EVENT_FIELD_SHOW_VBULLETIN_AUTHOR_PROFILE]));
        update_site_option('vbsso_ignore_membership', isset($_POST['vbsso_ignore_membership']));
        update_site_option(VBSSO_PLATFORM_FOOTER_LINK_PROPERTY, $_POST[VBSSO_PLATFORM_FOOTER_LINK_PROPERTY]);

        if (get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_USERGROUPS_URL, '')) {
    ...
    $show_vb_profile = get_site_option(VBSSO_NAMED_EVENT_FIELD_EDIT_PROFILE_IN_VBULLETIN, 1) ? 'checked' : '';
    $show_vb_profile_name = VBSSO_NAMED_EVENT_FIELD_EDIT_PROFILE_IN_VBULLETIN;
    $show_vb_profile_title = VBSSO_NAMED_EVENT_FIELD_EDIT_PROFILE_IN_VBULLETIN_TITLE;

    $ignore_membership = get_site_option('vbsso_ignore_membership', 1) ? 'checked' : '';
    $ignore_membership_name = 'vbsso_ignore_membership';
    $ignore_membership_title = 'Ignore Membership (Anyone can register through vBSSO)';

    $disabled_field = (get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LOGIN_URL, '')) ? 'disabled' : '';
...
EOD;
    ...
    <p>
        <input type="checkbox" id="{$ignore_membership_name}" name="{$ignore_membership_name}" value="1" {$ignore_membership} />
...
EOD;
    if (get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_USERGROUPS_URL, '')) {
        ...
        if (isset($vb_usergroups['error_string'])) {
            echo $vb_usergroups['error_string'];
        } else {
...
class vbsso_login_form extends WP_Widget {
    ...
    public function __construct() {
        ...
        add_action('wp_enqueue_scripts', array($this, 'vbsso_frontend_scripts'));
    }
    ...
    public function widget($args, $instance) {
        ...
        if (vbsso_get_platform_config_property(SHAREDAPI_PLATFORM_WORDPRESS, VBSSO_CONFIG_PROPERTY_OVERRIDE_LINKS, true) && get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LOGIN_VBULLETIN_URL, '') != '') {
            echo $before_widget;
            ...
            if (!is_user_logged_in()) {
                echo '<form action="' . get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_LOGIN_VBULLETIN_URL, '') . '" method="post">
                        <input class="input" type="text" name="vb_login_username" id="vb_username" style="padding:3px; margin-bottom: 3px; display: block;" accesskey="u" placeholder="' . __('Username or email address') . '" title="' . __('Username or email address') . '" />
                        <input class="input" type="password" name="vb_login_password" id="vb_password" style="padding:3px; margin-bottom: 3px; display: block;" placeholder="' . __('Password') . '" title="' . __('Password') . '" />

                        <input class="input" type="checkbox" name="cookieuser" value="1" id="vb_cookieuser" accesskey="c" />
                        <label for="vb_cookieuser" style="display: inline-block;">' . __('Remember me') . '</label>
                        <input class="button-primary" type="submit" value="' . __('Login') . '" accesskey="s" />

                        <input type="hidden" name="do" value="login" />
                    </form>';

                $metalinks .= $this->vbsso_wp_register('', '');
                ...
                echo '<div id="vbsso_avatar">' . get_avatar($user_ID, $size = '38') . '</div>';
    ...
    public function vbsso_frontend_scripts() {
    ...
    private function build_notifications() {
        ...
        if (is_array($vb_user_unread_stats) AND array_sum($vb_user_unread_stats) > 0) {
            $vb_stats_url = get_site_option(VBSSO_CURRENT_BLOG_NAMED_EVENT_FIELD_STATS_URL, '');
            $output .= '<li id="vbsso_stats_notifications">' . __('Notifications: ') . array_sum((array)$vb_user_unread_stats) . '</li>';
            $output .= '<div id="vbsso_stats_holder"><table>';
            $output .= ($vb_user_unread_stats[SHAREDAPI_EVENT_FIELD_STAT_PM]) ? '<tr><td>' . $vb_user_unread_stats[SHAREDAPI_EVENT_FIELD_STAT_PM] . '</td><td class="vbsso_stat_message"><a href="' . $vb_stats_url . SHAREDAPI_EVENT_FIELD_STAT_PM . '" rel="nofollow">Unread Private Messages</a></td></tr>' : '';
            $output .= ($vb_user_unread_stats[SHAREDAPI_EVENT_FIELD_STAT_VM]) ? '<tr><td>' . $vb_user_unread_stats[SHAREDAPI_EVENT_FIELD_STAT_VM] . '</td><td class="vbsso_stat_message"><a href="' . $vb_stats_url . SHAREDAPI_EVENT_FIELD_STAT_VM . '" rel="nofollow">Unread Visitor Messages</a></td></tr>' : '';
            $output .= ($vb_user_unread_stats[SHAREDAPI_EVENT_FIELD_STAT_FR]) ? '<tr><td>' . $vb_user_unread_stats[SHAREDAPI_EVENT_FIELD_STAT_FR] . '</td><td class="vbsso_stat_message"><a href="' . $vb_stats_url . SHAREDAPI_EVENT_FIELD_STAT_FR . '" rel="nofollow">Incoming Friend Requests</a></td></tr>' : '';
            $output .= ($vb_user_unread_stats[SHAREDAPI_EVENT_FIELD_STAT_PC]) ? '<tr><td>' . $vb_user_unread_stats[SHAREDAPI_EVENT_FIELD_STAT_PC] . '</td><td class="vbsso_stat_message"><a href="' . $vb_stats_url . SHAREDAPI_EVENT_FIELD_STAT_PC . '" rel="nofollow">Unread Picture Comments</a></td></tr>' : '';
            $output .= '</table></div>';
        }
        return $output;
    }

    private function vbsso_wp_register($before, $after) {
        if (!is_user_logged_in()) {
            $link = $before . '<a href="' . esc_url(apply_filters('register_url', site_url('wp-login.php?action=register', 'login'))) . '">' . __('Register') . '</a>' . $after;
        } else {
